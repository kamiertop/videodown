...
import {
  ...
  resumeDouyinBatch,
  stopDouyinBatch,
  type DouyinBatchState,
} from "../../lib/douyin/batchDownload.ts";

function statusBadge(state: DouyinBatchState): {text: string; badgeClass: string; spinning: boolean} {
 ...
 * 已完成的条目会从下载列表移除，几千个视频的收藏夹也只保留当前批的内容。
 */
export default function DouyinBatchStatusCard(): JSXElement {
  const state = douyinBatchState;
                          ...
                          <div class="flex items-center gap-2">
                            <Show when={batch().canResume}>
                              <button
                                  ...
                                  type="button"
                                  onClick={() => resumeDouyinBatch()}
                              >
                                继续批量
                              </button>
                            </Show>
                            <button class="btn btn-ghost btn-xs" type="button" onClick={dismissDouyinBatch}>关闭</button>
                          </div>
                        }
                    >
